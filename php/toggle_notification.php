...
try {
    $stmt = $conn->prepare("UPDATE bills SET notifications = NOT notifications WHERE id = ?");
    if (!$stmt) {
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            // Fetch the new status for the response message
            $stmt->close();
            $stmt = $conn->prepare("SELECT notifications FROM bills WHERE id = ?");
            if (!$stmt) {
                error_log("Prepare failed for status check: " . $conn->error);
                http_response_code(500);
                echo json_encode(["success" => false, "message" => "Query preparation failed: " . $conn->error]);
                ob_end_flush();
                exit;
            ...
            if ($row = $result->fetch_assoc()) {
                $status = $row['notifications'] ? 'enabled' : 'disabled';
                echo json_encode(["success" => true, "message" => "Notifications $status."]);
            } else {
                error_log("Bill not found after update: id=$id");
